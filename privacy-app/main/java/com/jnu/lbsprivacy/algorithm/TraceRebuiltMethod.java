...
public class TraceRebuiltMethod {
    ...
    private LatLng randomlyChoosePoiByCircle(double lat1,double lng1,int radius){
    ...
    //LatLng转Waypoints
    private WayPoint convert(LatLng sourceLatLng)
    {
        double m=sourceLatLng.latitude;
        double n=sourceLatLng.longitude;
        ...
        while ( i < k - 1)
        {
            for(j = i+1;j<k-2;j++)
            {
                ...
                if ((getSecond(wayPoint.get(i),wayPoint.get(j)) >time))
                {
                    ...
                    mywayPoint.add(mid);
                    latLng=randomlyChoosePoiByCircle(jlat,jlng,100);
                    while(poilat==latLng.latitude && poilng==latLng.longitude)
                    {
                        latLng=randomlyChoosePoiByCircle(jlat,jlng,100);

                    }
                    mywayPoint.add(convert(latLng));
                    poilat=latLng.latitude;
                    poilng=latLng.longitude;
                }
                else
                {
                    ...
                    while(j<k-2)
                    {
                        ...
                        if ((getSecond(wayPoint.get(i), wayPoint.get(j)) > time && (getDistance(ilat,ilng,wayPoint.get(j).getLatitude().doubleValue(),wayPoint.get(j).getLongitude().doubleValue())< meter)))
                        {
                            ...
                            latLng = randomlyChoosePoiByCircle(lat, lng, 100);
                            //判断POI是否重复
                            while(poilat==latLng.latitude && poilng==latLng.longitude)
                            {
                                latLng=randomlyChoosePoiByCircle(jlat,jlng,100);
